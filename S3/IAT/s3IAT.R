...
eval_reformat <- eval_iat %>%
  group_by(condition) %>%
  summarise_at(vars(eval_d), funs(mean(., na.rm=TRUE),sd(.,na.rm=TRUE), n()))
...
host_reformat <- host_iat %>%
  group_by(condition) %>%
  summarise_at(vars(host_d), funs(mean(., na.rm=TRUE),sd(.,na.rm=TRUE), n()))
...
ses_reformat <- ses_iat %>%
  group_by(condition) %>%
  summarise_at(vars(ses_d), funs(mean(., na.rm=TRUE),sd(.,na.rm=TRUE),n()))
