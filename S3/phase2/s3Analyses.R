...
# setwd("~/Desktop/scarcity-stereo-ms/study2AnalysesR/")
setwd("~/Documents/research/Amodio/scarcity/study2/2019_analyses/PART2")
...
data_long <- dplyr::select(data_long, subj_id, condition, rating)
...
stereo_data$group.f <- factor(stereo_data$group.f, levels = c("Scarcity", "Control"))
...
scarcity.stereo.gee <- geeglm(stereo.rating ~ group.f + stereotype.f + level.f +
                              ...
                              family = "gaussian",
                              corstr = "exchangeable")
summary(scarcity.stereo.gee)

scarcity.stereo.eval.gee <- geeglm(stereo.rating ~ group.f + stereotype.f + level.f + eval.rating +
                                group.f*stereotype.f + group.f*level.f + stereotype.f*level.f +
                                group.f*stereotype.f*level.f,
                              data = stereo_data,
                              id = subj_id,
                              family = "gaussian",
                              corstr = "exchangeable")
summary(scarcity.stereo.eval.gee)

# converting Wald
sprintf("%.4f",sqrt(as.numeric(sprintf("%.4f",
                                       as.numeric(summary(scarcity.eval.gee)[[6]][,'Wald'][4])))))
...
sd(stereo_high_scarcity$stereo.rating)
...
eval_data$group.f <- factor(eval_data$group.f, levels = c("Scarcity", "Control"))
...
scarcity.eval.gee <- geeglm(rating ~ group.f + level.f +
                              group.f*level.f,
                            data = eval_data,
                            id = subj_id,
                            family = "gaussian",
                            corstr = "exchangeable")
summary(scarcity.eval.gee)

eval_reformat <- eval_data %>%
  group_by(group.f, level.f) %>%
  summarise_at(vars(eval.rating), funs(mean(., na.rm=FALSE),std.error(., na.rm=FALSE)))
