...
data_long <- gather(data, condition, rating, CEH_like:SHL_aggressive, factor_key=TRUE)
...
ses_data <- stereo_data %>% 
  filter(stereotype=="ses")

host_data <- stereo_data %>% 
  filter(stereotype=="host")
...
ggplot(stereo_means, aes(x = level.f, y=mean, color=Group, fill = Group)) +
  ...
  geom_jitter(data = stereo_reformat, aes(colour = Group), shape = 20, alpha = .45,
              position = position_jitterdodge(jitter.width = .4, dodge.width = .91)) +
...
a <- 1:172
stereo_means_t <- tibble(a)
stereo_means_t$c.l <- stereo_low_control$stereo.rating
stereo_means_t$c.h <- stereo_high_control$stereo.rating
stereo_means_t$s.l <- stereo_low_scarcity$stereo.rating
stereo_means_t$s.h <- stereo_high_scarcity$stereo.rating
stereo_means_t <- stereo_means_t %>% 
  select(-a)

describe(stereo_means_t$c.l)
describe(stereo_means_t$c.h)
describe(stereo_means_t$s.l)
describe(stereo_means_t$s.h)

lessR::ttest(data = stereo_means_t,
             x = c.h,
             y = s.h,
